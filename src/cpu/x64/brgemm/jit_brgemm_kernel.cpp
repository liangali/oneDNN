...
private:
    ...
    const reg64_savable_t reg_per_k_group_size {regscratchpad_, r9};
    ...
    bool maybe_dot_product_with_per_k_scales(dim_t rd, dim_t bd, dim_t ld,
...
void jit_brgemm_kernel_t<Wmm>::read_params() {
    ...
    if (brg.with_wei_scales) {
        ...
        if (brg.is_per_k_wei_scales) {
            mov(reg_per_k_group_size, brg.wei_scale_k_group_size);
            reg_per_k_group_size.save();
        }
    }
...
void jit_brgemm_kernel_t<Wmm>::store_accumulators_apply_post_ops(dim_t bd_block,
    ...
    // In case of per K scales, the conversion is done during gemm_microkernel.
    bool dq2ps_cvt_done = brg.has_per_k_scales();

    // When per K src scales are active, they are applied in the microkernel.
    ...
    if (brg.has_per_k_scales() && !brg.is_tmm) reg_aux_D.restore();
...
void jit_brgemm_kernel_t<Wmm>::apply_compensation(
    ...
    // For the per K scales compensation is done in gemm microkernel.
...
void jit_brgemm_kernel_t<Wmm>::store_accumulators(dim_t bd_block2,
    ...
    if (brg.is_tmm) {
        ...
        /**
         * Optionally applies per K weights/src scales for AMX.
        ...
        */
        auto maybe_apply_per_k_scales_amx
                    ...
                    if (do_accum_ops) {
                        if (skip_accumulation) {
                            ...
                            for (dim_t bd = 0; bd < adj_bd_block; bd++) {
                                ...
                                maybe_apply_per_k_scales_amx(ldb, vreg_acc);
                            }
                        }
 ...
 * Handles both per K wei scales and per K src scales (or both).
 ...
 */
template <typename Wmm>
bool jit_brgemm_kernel_t<Wmm>::maybe_dot_product_with_per_k_scales(dim_t rd,
        dim_t bd, dim_t ld, bool is_ld_tail, const Vmm &acc,
        const Vmm &vmm_B_wei, const Vmm &vmm_A_bcst) {
    if (!brg.has_per_k_scales()) return false;
    ...
    if (brg.is_per_k_wei_scales) {
        ...
        reg_per_k_group_size.restore();

        xor_(rdx, rdx);
        mov(rax, reg_grouped_k);
        add(rax, rd);
        div(reg_per_k_group_size);

        xor_(rdx, rdx);
        mov(reg_per_k_group_size,
                brg.load_dim * types::data_type_size(brg.dt_wei_scales));
        mul(reg_per_k_group_size);
...
void jit_brgemm_kernel_t<Wmm>::gemm_microkernel(dim_t bd_block2,
    ...
    auto maybe_restore_vmm_inp_shift = [&]() {
        if (brg.req_s8s8_compensation && brg.has_per_k_scales()) {
            mov(reg_s8_input_shift, 128);
            uni_vpbroadcastb(vmm_inp_shift(), reg_s8_input_shift.cvt8());
        }
    };
    ...
    auto load_B = [this, is_ld_tail](dim_t vmm_load_idx, dim_t rd, dim_t ld) {
        if (brg.has_per_k_scales()) reg_aux_B.restore();
    ...
    for (dim_t rd = 0; rd < rd_loop; rd += brg.rd_step) {
        if (brg.n_bcast_1_load) {
            ...
            for (dim_t ld = 0; ld < ld_block2; ld++) {
                ...
                for (dim_t bd = bd_b; bd < bd_e; bd++) {
                    auto vmm = accm(ld_block2, bd, ld);
                    if (maybe_dot_product_with_per_k_scales(
    ...
    // maybe_dot_product_with_per_k_scales uses rax, rbp, r10 and r11
    if (brg.has_per_k_scales()) {
        reg_aux_A.restore();
        reg_aux_B.restore();
...
void jit_brgemm_kernel_t<Wmm>::bs_loop(dim_t bd_block2, bool is_bdb_tail,
    ...
    if (brg.alpha != 0.f && !skip_accumulation) {
        ...
        if (brg.brgattr.max_bs > 1) {
            ...
            if (brg.has_per_k_scales()) reg_BS_loop.save();
        } else if (brg.has_per_k_scales())
            reg_aux_D.save();
        L_aligned(BS_loop_label, 64);
        {
            ...
            if (brg.brgattr.max_bs > 1) {
                if (brg.has_per_k_scales()) reg_BS_loop.restore();
                dec(reg_BS_loop);
                cmp(reg_BS_loop, 0);
